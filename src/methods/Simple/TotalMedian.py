...
class TotalMedian(Base):

    def train_test_save(self, nan_percent_value):
        super().train(TotalMedian.get_train_params(), TotalMedian.fill_nan)
        super().test(TotalMedian.get_train_params(), TotalMedian.fill_nan_test)
        super().save_result(TotalMedian.get_name(), nan_percent_value)
    ...
    def fill_nan(temp_df: pd.DataFrame, _):
        ...
        temp_median = np.nanmedian(temp_array).sum()
        filled_nan = np.nan_to_num(temp_array, nan=temp_median)
        temp_nan_index = np.where(np.isnan(temp_array))[0]
        return pd.DataFrame({"predicted_usage": filled_nan[temp_nan_index].squeeze()},
                            index=final_filled_nan_index.squeeze()), user_id, temp_median
